...
double **convertPythonToC_X(PyObject *self, PyObject *args){ /* This function converts PyObject to C object for the data points */
    PyObject *X = NULL, *curr_cordinate_item = NULL, *curr_item_list = NULL, *innerList = NULL;
    int checklen = 0, checkleninner = 0, j = 0, ind = 0;
    double curr_cordinate_index = 0.0, *nextVectorToRead = NULL, **X_c = NULL;
    if (!PyArg_ParseTuple(args, "O", &X) ||!PyList_Check(X) ) {
    ...
    innerList = PyList_GetItem(X, 0), checkleninner = PyObject_Length(innerList);
    if (checkleninner < 0) {
        PyErr_SetString(PyExc_Exception, "An Error Has Occurred\n");
        return NULL;
    }
    n = (int)checklen,d = (int)checkleninner;
    X_c =(double **) malloc(n * sizeof(double *));
    if (X_c == NULL) {
        PyErr_SetString(PyExc_Exception, "An Error Has Occurred\n");
        return NULL;
    }
    for (j = 0; j < n; j++) { /* Creating a matrix of datapoints */
        nextVectorToRead =(double *) malloc(d * sizeof(double));
        if (nextVectorToRead == NULL) { /* check if allocation did not succeed */
...
static PyObject *symnmf(PyObject *self, PyObject *args) {
    PyObject *H = NULL, *W = NULL, *resultP = NULL;
    double **RES_H = NULL, **W_c = NULL, **H_c = NULL;
    int checkH = 0, checkW = 0, k = 0, n_curr = 0;
    if (!PyArg_ParseTuple(args, "OOii", &H, &W, &n_curr, &k) || 
        !PyList_Check(H) || !PyList_Check(W)) {
        PyErr_SetString(PyExc_Exception, "An Error Has Occurred\n");
        return NULL;
    }
