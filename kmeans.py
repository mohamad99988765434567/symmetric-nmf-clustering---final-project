...
def update_centroid(vectors, centroid_index, vectors_cluster, d, n):
    ...
    new_centroid = [0 for i in range(d)] # init
...
def clusters(k, filename):
    ...
    d = len(vectors[0])# Checking if all datapoints are of the same dimension
    for i in range(n):
        if len(vectors[i]) != d:
            print(ERROR_GENERAL)
            return 1
    for i in range(k):# Initialising centroids to the first k datapoints
        centroids[i] = vectors[i][:]
        cluster_per_vector[i] = i
    while (check_under_epsilon == 1) and (curr_iter < max_iter):
        curr_iter += 1
        for i in range(n):# Assign each data point to the closest cluster
            new_cluster = update_vector_cluster(vectors[i], centroids)
            cluster_per_vector[i] = new_cluster
            
        check_under_epsilon = 0 # Update the centroids and check if the difference is under epsilon
        for i in range(k):
            ...
            if difference_new_and_old_centroids == -1:# In case vectors are not in the same length
                print(ERROR_GENERAL)
                exit(1)
